import { auth } from "@clerk/nextjs/server";
import { NextResponse } from "next/server";
import { supabaseAdmin } from "@/lib/supabase/admin";

export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    const { userId } = await auth();
    if (!userId) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    const id = params.id;
    const body = await req.json();
    const mcpId: number | undefined = body?.mcpId;
    const position: number | undefined = body?.position;
    if (!mcpId) return NextResponse.json({ error: "mcpId required" }, { status: 400 });

    const { data: pkg, error: pkgErr } = await supabaseAdmin
      .from("packages")
      .select("id, user_id")
      .eq("id", id)
      .single();
    if (pkgErr || !pkg) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (pkg.user_id !== userId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

    const { error: insErr } = await supabaseAdmin
      .from("package_items")
      .insert({ package_id: id, mcp_id: mcpId, position: position ?? 0 });
    if (insErr) throw insErr;

    // bump updated_at
    await supabaseAdmin.from("packages").update({ updated_at: new Date().toISOString() }).eq("id", id);

    return NextResponse.json({ ok: true }, { headers: { "Cache-Control": "no-store" } });
  } catch (e: any) {
    console.error("POST /api/packages/[id]/items", e);
    return NextResponse.json({ error: "Unexpected error" }, { status: 500 });
  }
}

